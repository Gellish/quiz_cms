...
class Lcommon_exam {
	...
	//Coure List Individual View
	public function course_list_individual_view($limit,$page,$links)
	{
		$CI =& get_instance();
		$CI->load->model('front/Common_exams');
		
		$course_list = $CI->Common_exams->retrieve_course_list_for_limit($limit,$page,$links);
		...
		$courseList = $CI->parser->parse('front_view/common_exam/all_course',$data,true);
		return $courseList;
	}
	...
	public function retrieve_chapter_list( $course_id )
	{
		...
		$chapter_list = $CI->Common_exams->get_chapters( $course_id );

		if ($chapter_list) {
			$total_ques = 0;
			foreach($chapter_list as $key=>$value)
			{
				$total_ques = $total_ques + $value['total_question'];
			}
		}

		$course_list = $CI->Common_exams->retrieve_course_list_by_id($course_id);
		...
		$data = array(
			'title' => 'Chapters List',
			'course_id' => $course_id,
			'total_ques' => (!empty($total_ques)?$total_ques:null),
			'course_name' => $chapter_list[0]['course_name'],
			'course_details' => $course_list[0]['course_details'],
			'course_image' => $course_list[0]['image'],
		...
		$chapterList = $CI->parser->parse('front_view/common_exam/ques_set_create',$data,true);
		return $chapterList;
	}
	...
	public function get_common_exam_view( $question_ids )
	{
		...
		if(empty($question_ids)){
			$CI->session->set_userdata(array('warning_message'=>"No Questin Found for Your Selected Chapters"));
			redirect(base_url('front/Common_exam/chapter_list'));exit();
		}else{
			sort($question_ids);
		...
		$question_data = $CI->Common_exams->get_questions_and_options( $qstn_first_id );			
		
		$btn_previous ="";
		$btn_next ="show";
		
		if(isset($CI->session->userdata['exam_related_data']['course_name'])){
			$course_name = $CI->session->userdata['exam_related_data']['course_name'];
		 }else{$course_name ="";}
		...
		$questionList = $CI->parser->parse('front_view/common_exam/exam_start_view',$data,true);
		return $questionList;
	}
	...
	public function submit_common_exam_view( $option_id )
	{
		...
		if( $option_id !=''){
			...
			if(empty($sess_result_array)){
				//Change here 
				$result_data = array('result_data');//Change  => array()
				//Change here
		...
		//Change here array('exam_data' => '')
		$exam_data = array('exam_question_ids'=>'','current_index'=>'','last_index'=>'','exam_start_time'=>'');
		//Changed here
		...
		$question_data = $CI->Common_exams->get_questions_and_options( $expected_qstn_id );
	...
	public function common_exam_result_processing( )
	{
		...
		$course_id = $exam_related_data['course_id'];
		...
		$data = array(
			...
			'questions_id' 	=> json_encode($question_ids,JSON_FORCE_OBJECT),
			'answers_id' 	=> json_encode($result_data,JSON_FORCE_OBJECT)
		);
		
		$CI->Common_exams->insert_exam_result_details( $data );
		
		// UPDATE SCHEDULE NOTIFICATION TABLE 
		$update_data = array(
			'status' 	=> 2
		);
		
		$CI->Common_exams->update_schedule_exam_table( $exam_id,$update_data );
		...
		$CI->Common_exams->entry_exam_result( $data1 ); 
/* 		
		// View Data
		$result_data = array(
			'course_name' => $course_name,
		...
		// Unset Session Array;
			...
			$CI->session->unset_userdata($exam_data);
			$another_exam_data =array('course_id'=> '','no_of_question' => '','exam_id'=> '');
		...
		redirect(base_url('front/Common_exam/view_common_exam_result'));exit();
		
	}
	public function final_exam_result_view( $exam_id )
	{
		$CI =& get_instance();
		$CI->load->model('front/Common_exams');	
		
		$question_data = $CI->Common_exams->get_exam_question_data( $exam_id );

		$question_ids = json_decode( $question_data[0]['questions_id'],true);
		$result_data =  json_decode( $question_data[0]['answers_id'],true);

		
		$exam_result_data = $CI->Common_exams->get_exam_result_data( $exam_id );
		$course_id = $exam_result_data[0]['course_id'];
		...
		$total_correct_answer = $this->correct_answer_counter( $final_result );
		$total_answered = count($result_data);
		$course_name = 	$CI->Common_exams->get_course_name( $course_id );
		$chapter_ids = 	$CI->Common_exams->get_chapter_ids( $exam_id );
		$all_chapter_id = array();
		if(!empty($chapter_ids)){$all_chapter_id = (array) json_decode( $chapter_ids[0]['chapter_ids'] );}
		$chapters_name = $CI->Common_exams->get_chapter_names( $all_chapter_id );
		...
		$html_summary_result = $CI->parser->parse('front_view/common_exam/summary_result',$results_data,true);

		$i = 0;
		foreach ($final_result as $key=>$value){$i++;
			$final_result[$key]['sl'] = $i;
			$final_result[$key]['exam_&_ques_id'] = $exam_id."-".$value['question_id'];
		}
		$view_data = array(
		...
		$html_view = $CI->parser->parse('front_view/common_exam/exam_result',$view_data,true);
	...
	{	
		...
		if(!empty($question_ids)){
			for( $i=0; $i<count($question_ids); $i++ ){
				...
				$original_answer_array =$CI->Common_exams->get_question_single_answer_sheet( $question_id );
	...
	public function individual_question_view( $exam_id,$ques_id )
	{
		$CI =& get_instance();
		$CI->load->model('front/Common_exams');
		$answered_result = $CI->Common_exams->get_user_answered_result( $exam_id );
		$answered_array = (array) json_decode($answered_result[0]['answers_id'],true);
		$answered_option_ids = $this->array_index_searcher( $answered_array,$ques_id );

		$question_and_option = $CI->Common_exams->get_questions_and_options( $ques_id );
		...
		$original_answer = $CI->Common_exams->get_question_single_answer_sheet( $ques_id );
		...
		if($matching_result){
			...
			$wrong_answered_ids = $CI->Common_exams->get_user_selected_wrong_answer( $answered_option_ids );
