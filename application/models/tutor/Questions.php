...
class Questions extends CI_Model {
	...
	public function count_question_list(){
		...
		$this->db->where(array('c.status'=>1,'d.status'=>1,'e.status'=>1,'f.creator_id'=>$tutor_id));
		$query = $this->db->get();
	...
	public function retrieve_question_list($limit,$page){
		...
		$this->db->join('class_add e', 'e.class_id = d.class_id');
		$this->db->join('question_creator_relation f', 'f.question_id = a.question_id');
		$this->db->where(array('c.status'=>1,'d.status'=>1,'e.status'=>1,'f.status'=>1,'f.creator_id'=>$tutor_id)); 
	...
	//
	public function retrieve_question_list_by_chapter_id($chapter_id){
		$CI =& get_instance();
		$tutor_id =  $CI->session->userdata('user_id');
